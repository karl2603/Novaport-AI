...
export const dict = {
  en: {
    ...
    card1_title: 'Data Extraction', card1_sub: 'Transform unstructured commercial PDFs into precise JSON schemas instantly. Perfect mapping of line items and incoterms.',
    ...
    node_select: 'Select Data Extraction Nodes', node_entities: 'Shipper & Consignee', node_logistics: 'Ports & Routing', node_products: 'Line Items & Values',
...
function App() {
  ...
  const handleFileUpload = async (e, endpoint) => {
    ...
    try {
      const response = await axios.post(`https://novaport-ai-backend-i6jw.onrender.com/api/${endpoint}`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
  ...
  const handleHSCodeGenerate = async (e) => {
    ...
    setLoading(true); setError('');
    try {
      const response = await axios.post(`https://novaport-ai-backend-i6jw.onrender.com/api/hscode`, {
